...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    xmlns = "http://docbook.org/ns/docbook"
		...
		xmlns:xlink="http://www.w3.org/1999/xlink"
                exclude-result-prefixes="exsl db d #default"
                version="1.0">
...
# Standards (OASIS).
#
# Release: $Id: db4-upgrade.xsl 8885 2010-08-27 02:52:15Z dcramer $
#
...
<xsl:template match="/">
  ...
  <xsl:comment>
    ...
    <xsl:value-of select="$version"/> (with Motive additions)
    <xsl:text> </xsl:text>
  </xsl:comment>
  <xsl:text>&#10;</xsl:text>

...
<xsl:import href="../../../docbook-xsl/1.75.2/docbook/common/addns.xsl"/>
...
<xsl:param name="profile.author_condition_1" select="''"/>
<xsl:param name="profile.author_condition_2" select="''"/>
...
<xsl:param name="profile.security" select="''"/>
  <xsl:param name="profile.revisionflag">
    <xsl:choose>
      <xsl:when test="$profile.security = 'reviewer' or $profile.security = 'writeronly'">deleted;added;changed;off</xsl:when>
      <xsl:otherwise>added;changed;off</xsl:otherwise>
    </xsl:choose>
  </xsl:param>
...
<xsl:template match="/">
  ...
  <xsl:template match="@revisionflag" mode="profile">
      <xsl:if test="$profile.security = 'reviewer' or $profile.security = 'writeronly'">
        <xsl:copy-of select="."/>
      </xsl:if>
  </xsl:template>

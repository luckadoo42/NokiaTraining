...
<xsl:template match="*" mode="profile">
  ...
  <!-- DWC: Stuff added -->
  <xsl:variable name="author_condition_1.content">
    <xsl:if test="@author_condition_1">
      <xsl:call-template name="cross.compare">
        <xsl:with-param name="a" select="$profile.author_condition_1"/>
        <xsl:with-param name="b" select="@author_condition_1"/>
      </xsl:call-template>
    </xsl:if>
  </xsl:variable>
  <xsl:variable name="author_condition_1.ok" select="not(@author_condition_1) or not($profile.author_condition_1) or
    $author_condition_1.content != '' or @author_condition_1 = ''"/>
  
  <xsl:variable name="author_condition_2.content">
    <xsl:if test="@author_condition_2">
      <xsl:call-template name="cross.compare">
        <xsl:with-param name="a" select="$profile.author_condition_2"/>
        <xsl:with-param name="b" select="@author_condition_2"/>
      </xsl:call-template>
    </xsl:if>
  </xsl:variable>
  <xsl:variable name="author_condition_2.ok" select="not(@author_condition_2) or not($profile.author_condition_2) or
    $author_condition_2.content != '' or @author_condition_2 = ''"/>
  <!-- DWC: End stuff added -->
  ...
  <xsl:variable name="security.ok" select="not(@security) or not($profile.security) or
                                           $security.content != '' or @security = '' or ($profile.security='reviewer' and not(@security='writeronly') and not(@security='unconscious'))"/>
  ...
  <xsl:if test="$arch.ok and 
                $author_condition_1.ok and
                $author_condition_2.ok and 
                $audience.ok and 
                $condition.ok and 
                $conformance.ok and 
                $lang.ok and 

...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
 ...
 -->
	...
	<xsl:variable name="covertitle">
		...
		<xsl:message>cover.xsl defines covertitle as <xsl:value-of select="//processing-instruction('covertitle')"/></xsl:message>
	</xsl:variable>
		...
		<fo:page-sequence hyphenate="{$hyphenate}" master-reference="motive.component.coversequence"
			...
			<fo:flow flow-name="xsl-region-body">
				...
				<fo:block-container id="{$id}" absolute-position="fixed" top="2.8in" right=".85in"
					left="3.5in" z-index="-1" text-align="{$motive.component.cover.text.align}">
					  <!-- changed left from 4.5 to 3.5 to prevent squishing of numbers in XEP; similarly, below in fo:block changed width from 3 to 4 -->
					<fo:block width="4in" color="silver" font-size="250pt">
				...
				<!-- page number  (original: this just outputs page num, instead of rest of footer, here
				...
				I had to hardcode this into a block-container that positions it absolutely; without that, it shows up top of page
				-->
				 <fo:block-container absolute-position="fixed" top="10.05in" left="1.5in" width="6in"> 
					...
					<fo:block-container absolute-position="fixed"
						...
						<fo:table table-layout="fixed">
							...
							<fo:table-body>
								...
								<xsl:message>The issue number defined for this book is: [<xsl:value-of select="$ISSUE"/>]</xsl:message>
								<xsl:if test="$ISSUE !=''">
									<fo:table-row height=".22in">
										<fo:table-cell display-align="after">
											<fo:block width="{$motive.cover.title.block.width}"
...
<!--  Test not needed. It's always by Alcatel, or Nokia in this folder -->
<!--     <xsl:if test="$authors"> -->
      ...
      <xsl:element name="rx:meta-field">
        ...
        <xsl:attribute name="value">Nokia

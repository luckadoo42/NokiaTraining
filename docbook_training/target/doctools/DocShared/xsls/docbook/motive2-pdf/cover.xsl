...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
 ...
 -->
	...
	<xsl:param name="cover.path">
		<xsl:choose>
			<xsl:when test="$xep.extensions !='0'"><xsl:value-of select="concat($common.graphics.path, '/alcatel2/cover.pdf')"/></xsl:when>
			<xsl:otherwise><xsl:value-of select="concat($common.graphics.path, '/alcatel2/cover.svg')"/></xsl:otherwise>
		</xsl:choose>
	</xsl:param>
...
select="concat($common.graphics.path, '/motive.chapter.cover.svg')"/> -->
	...
	<xsl:param name="motive.cover.title.font-family">Trebuchet MS, Trebuchet</xsl:param>

	<xsl:param name="motive.cover.subtitle.font-family">Trebuchet MS, Trebuchet</xsl:param>
	...
	<xsl:param name="motive.cover.title.left.margin">.98in</xsl:param>
	<xsl:param name="motive.cover.subtitle.left.margin">.98in</xsl:param>
	...
	<xsl:param name="motive.cover.productnumber.font.size">25pt</xsl:param>
		...
		<xsl:variable name="title">
			<xsl:choose>
				...
				<xsl:when test="$covertitle !=''">
					...
					<xsl:message>JCBG-823 title is now <xsl:value-of select="$covertitle"/></xsl:message>
				</xsl:when>
...
<!-- inserting cover art via a block-container w abs pos, for jcbg-76; this works in fop
	but produces an X for 'graphic not found' in XEP
...
-->
					...
					<!-- place the ALCATEL-LUCENT 55xx -->
					<fo:block-container absolute-position="fixed"
						text-align="{$motive.cover.text.align}" top="5.40in"
						left="{$motive.cover.title.left.margin}" right="0in">
						<!-- top is from top of page to start of text, i think -->
								...
								<fo:table-row>
									...
									<fo:table-cell display-align="after">
										<fo:block width="{$motive.cover.title.block.width}"
											...
											ALCATEL-LUCENT <xsl:value-of
												select="/book/bookinfo/productnumber"/>
										</fo:block>
									</fo:table-cell>
								</fo:table-row>
							</fo:table-body>
					...
					<!-- now the rest of cover text 
			Had to use a new block-container, because could not position 2nd line accurately otherwise-->

					<fo:block-container absolute-position="fixed"
						text-align="{$motive.cover.text.align}" top="5.74in"
						left="{$motive.cover.title.left.margin}" right="0in">
						<fo:table table-layout="fixed">
							<fo:table-column column-width="{$motive.cover.title.block.width}"/>
							<fo:table-body>
								...
								<fo:table-row >
									<fo:table-cell display-align="after">
										<fo:block width="{$motive.cover.title.block.width}"
											color="black" font-weight="bold"
											font-family="{$motive.cover.title.font-family}"
											font-size="{$motive.cover.prodname.font.size}">
											<xsl:call-template name="string.upper">
												<xsl:with-param name="string">
												<xsl:value-of select="/book/bookinfo/productname"
												/> | <xsl:value-of
												select="/book/bookinfo/releaseinfo"/>
												</xsl:with-param>
											</xsl:call-template>
										</fo:block>
									</fo:table-cell>
								...
								<fo:table-row height=".22in">
									<fo:table-cell display-align="after">
										<fo:block width="{$motive.cover.title.block.width}"
											...
											<xsl:if test="/book/bookinfo/issuenum !=''">ISSUE:
												<xsl:value-of select="/book/bookinfo/issuenum"/> | </xsl:if>

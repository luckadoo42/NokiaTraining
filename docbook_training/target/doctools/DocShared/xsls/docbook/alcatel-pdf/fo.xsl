...
<xsl:param name="admon.graphics.path" select="concat($common.graphics.path, '/alcatel/')"/>
  ...
  <fo:block>
	...
	<fo:block><fo:leader/></fo:block>
      ...
      <xsl:when test="$double.sided != 0 and 
        ...
        <fo:block margin-top="2pt"><xsl:value-of select="//productnumber[1]"/><xsl:text> </xsl:text><xsl:value-of select="//productname[1]"/><xsl:text> </xsl:text><xsl:apply-templates select="/*[1]" mode="title.markup"/><xsl:if test="//releaseinfo[1]">, <xsl:value-of select="//releaseinfo[1]"/></xsl:if></fo:block>
...
<fo:block xmlns:fo="http://www.w3.org/1999/XSL/Format" xsl:use-attribute-sets="book.titlepage.recto.style" text-align="left" font-size="10pt" letter-spacing=".25em" start-indent="5mm" font-weight="normal" font-family="{$title.font.family}">

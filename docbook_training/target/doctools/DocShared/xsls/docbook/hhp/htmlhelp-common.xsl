...
<!-- ********************************************************************
     $Id: htmlhelp-common.xsl,v 1.16 2008/12/18 03:36:24 dcramer Exp $
     ******************************************************************** -->
...
<HTML>&lf;
 <HEAD>&lf;
<title><xsl:value-of select="$help.title"/></title>&lf;
</HEAD>&lf;
 <BODY>&lf;
...
<xsl:template match="part|reference|preface|chapter|bibliography|appendix|article|glossary"
  ...
  <xsl:if test="article|reference|preface|chapter|appendix|refentry|section|procedure|sect1|simplesect|bibliodiv">
    <UL>&lf;
      <xsl:apply-templates
	select="article|reference|preface|chapter|appendix|refentry|section|procedure|simplesect|sect1|bibliodiv"
	mode="hhc"/>
    </UL>
  </xsl:if>
</xsl:template>

<xsl:template match="section|procedure|simplesect" mode="hhc">
	<xsl:if test="not(self::procedure) or $procedures.in.toc != '0'">
  <xsl:if test="$htmlhelp.hhc.show.root != 0 or parent::*">
    <xsl:call-template name="hhc.entry"/>
  </xsl:if>
  <xsl:if test="section[count(ancestor::section) &lt; $htmlhelp.hhc.section.depth]|simplesect[count(ancestor::section) &lt; $htmlhelp.hhc.section.depth]|refentry|procedure">
    <UL>&lf;
      <xsl:apply-templates select="section|procedure|refentry|simplesect" mode="hhc"/>
    </UL>
  </xsl:if>
	</xsl:if>
</xsl:template>
...
<xsl:template xmlns="http://www.w3.org/1999/xhtml" name="write.indexterm">
  <xsl:param name="text"/>
...
<xsl:template xmlns="http://www.w3.org/1999/xhtml" match="bibliodiv/title">
  <h3 class="{name(.)}">

...
<!-- ********************************************************************
     $Id: chunk-common.xsl,v 1.28 2009/11/17 22:24:02 dcramer Exp $
     ********************************************************************
...
-->
  ...
  <xsl:choose>
	  <!-- dwc: Adding support for stop-chunking PI --> 
    ...
    <xsl:choose>
      ...
      <xsl:when test="not(parent::*) and $root.filename != ''">
        <xsl:value-of select="$root.filename"/>
		  <!-- DWC: Making it so I can specify the whole file name with the root.filename param. I need it to be index.htm to make OLCS happy -->
...
<xsl:template name="make.olink.href">
  ...
  <!-- DWC: By default, don't hyperlink olinks -->
...
<meta http-equiv='Content-Type' content='text/html; charset={$chunker.output.encoding}'/>
    ...
    <xsl:if test="//legalnotice                   and not($generate.legalnotice.link = 0)                   and not($html.head.legalnotice.link.types = '')">
	  ...
	  <xsl:if test="$tooltip.glossterms != 0"><script type="text/javascript" src="{$common.files}/js/overlib.js"><xsl:comment>overLIB (c) Erik Bosrup</xsl:comment></script></xsl:if>
 	  <script type="text/javascript" src="{$common.files}/js/motive.js"><xsl:comment> </xsl:comment></script>
...
<xsl:template name="navig.content">
    ...
    <xsl:choose>
        ...
        <xsl:otherwise>
		<!-- DWC: Adding normalize-space() here to avoid a line break,
		because I'm writing out this using document.write. Why am I
		writing this out with document.write you ask? Because I need
		to hide it from the Eclipse help search mechanism which uses
		the first bit of text it can find as the summary for the
		search results.  -->
            <xsl:value-of select="normalize-space($navtext)"/>
        </xsl:otherwise>
    </xsl:choose>
</xsl:template>
...
<div style="text-align: right; padding: 1em;"><span id="togglePlaceholder" style="font-weight: bold"><xsl:comment> IE needs this not to be &lt;span/> </xsl:comment></span></div>

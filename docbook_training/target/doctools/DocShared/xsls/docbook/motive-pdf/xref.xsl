...
<!-- ********************************************************************
     $Id: xref.xsl,v 1.7 2009/10/21 22:53:21 dcramer Exp $
     ********************************************************************
...
<!-- ==================================================================== -->
 ...
 <!-- adding for JCBG-89 -->
  <xsl:param name="fail.on.bad.olink">yes</xsl:param>
...
<xsl:template match="xref" name="xref">
  ...
  <xsl:choose>
	...
	<!-- DWC: Hacking this so we don't get "on page" with
	xrefs to steps because that looks stupid.  -->
    <xsl:when test="not(starts-with(normalize-space($xrefstyle), 'select:') 
                  and (contains($xrefstyle, 'page')
                       or contains($xrefstyle, 'Page')))
                  and ( $insert.xref.page.number = 'yes' 
                     or $insert.xref.page.number = '1')
                  and not(local-name($target) = 'step') and not(@role = 'nopage') and not(local-name($target) = 'para')">
...
<xsl:template match="*" mode="xref-to">
  ...
  <xsl:if test="$verbose and $fail.on.bad.olink='yes'">
    <xsl:message terminate="yes">
      <xsl:text>Don't know what gentext to create for xref to "</xsl:text>
      <xsl:value-of select="name(.)"/>
      <xsl:text>" element with id or xml:id of "</xsl:text>
      <xsl:value-of select="(@id|@xml:id)[1]"/>
      <xsl:text>". Ensure that the element being referenced has an associated title (JCBG-573).</xsl:text>
    </xsl:message>
  </xsl:if>
  <xsl:text>???</xsl:text>
</xsl:template>
...
<xsl:template match="olink" name="olink">
  ...
  <xsl:choose>
    ...
    <xsl:when test="@targetdoc or @targetptr" >
      ...
      <!-- DWC: Put quotes around "hottext -->
      <xsl:call-template name="gentext.startquote"/><xsl:copy-of select="$hottext"/><xsl:call-template name="gentext.endquote"/>
...
<!--  For JCBG-76, calling the inline template for italics, instead of hardcoding, to get the right font family...
  <fo:inline font-style="italic">
    <xsl:value-of select="$docname"/>
  </fo:inline> -->

  <xsl:call-template name="inline.italicseq">
   <xsl:with-param name="content" select="$docname"/>
  </xsl:call-template>
...
<xsl:template name="title.xref">
  ...
  <xsl:choose>
    ...
    <xsl:otherwise>
<!-- dwc: Commenting out. We don't want this. -->
<!--       <fo:inline font-style="italic"> -->
        <xsl:apply-templates select="$target" mode="title.markup"/>
<!--       </fo:inline> -->
    </xsl:otherwise>
...
<xsl:template match="*" mode="page.citation">
  ...
  <fo:basic-link internal-destination="{$id}"
                 xsl:use-attribute-sets="xref.properties">
<!-- JCBG-1359, allow page citations to break by commenting out the keep-together
...
<!-- also JCBG-1359, commenting out the end tag   </fo:inline> -->
  </fo:basic-link>
</xsl:template>

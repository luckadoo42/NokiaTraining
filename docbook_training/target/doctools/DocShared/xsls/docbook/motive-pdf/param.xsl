...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  xmlns:src="http://nwalsh.com/xmlns/litprog/fragment" exclude-result-prefixes="src" version="1.0">
  <!-- This file is generated from param.xweb; do not edit this file! -->

  <!-- ********************************************************************
     $Id: param.xsl,v 1.56 2009/12/02 21:56:23 dcramer Exp $
     ********************************************************************
  ...
  <xsl:param name="body.font.family"
    select="'CheltenhamCdITC,sans-serif,ZapfDingbats,LucidaSansUnicode,KozMinPro-Regular-Acro'"/>
  ...
  <xsl:param name="callout.graphics.number.limit" select="'99'"/>

  <xsl:param name="callout.graphics.path"
    select="translate(concat($common.graphics.path,'/callouts/'),'\','/')"/>
  <!-- sanitizing the slashes in this for fop, JCBG-76, to fix otherwise bad paths for callout svg files -->
  ...
  <xsl:param name="draft.watermark.image" select="concat($common.graphics.path, '/draft.png')"/>
  ...
  <xsl:attribute-set name="formal.title.properties" use-attribute-sets="normal.para.spacing">
    <xsl:attribute name="font-weight">
      <xsl:choose>
        <xsl:when test="self::figure or self::table">bold</xsl:when>
  ...
  <xsl:param name="generate.section.toc.level" select="100"/>

  <xsl:param name="generate.toc"> /appendix nop article/appendix nop /article toc,title book toc
    /chapter nop part nop /preface nop qandadiv toc qandaset toc reference toc /sect1 nop /sect2 nop
    /sect3 nop /sect4 nop /sect5 nop /section nop set toc,title </xsl:param>
  ...
  <!-- jcbg-76: for fop, turn on hyphenate.verbatim (line breaking chars) only if using xep) -->
  <xsl:param name="hyphenate.verbatim">
    <xsl:value-of select="$xep.extensions"/>
  </xsl:param>
  <!-- so if xep.extensions =1, then hyphenate.verbatim=1 and you get line breaking chars-->
  ...
  <xsl:attribute-set name="index.page.number.properties"> </xsl:attribute-set>
  ...
  <xsl:attribute-set name="itemizedlist.properties" use-attribute-sets="list.block.properties"> </xsl:attribute-set>
  <xsl:attribute-set name="itemizedlist.label.properties">
    <xsl:attribute name="font-size">7pt</xsl:attribute>
    <xsl:attribute name="padding-top">.2em</xsl:attribute>
    <!-- was .75em, changed to .2em for JCBG-76 fop and to better align bullets -->
  </xsl:attribute-set>
  <xsl:param name="itemizedlist.label.width" select="'1em'"/>
  ...
  <xsl:attribute-set name="list.block.spacing">
    ...
    <xsl:attribute name="space-after.optimum">
      <xsl:choose>
        <xsl:when
		  ...
		  (parent::para or 
		  parent::entry or
		  parent::revdescription) and
		  not(normalize-space(following-sibling::node()) = '')"
          >1em</xsl:when>
    ...
    <!-- Commenting out these lines to avoid a mysterious "circular reference to attribute set" error -->
    ...
    <!-- 	  parent::entry or -->
    <!-- 	  parent::revdescription) and -->
    <!-- 	  not(normalize-space(following-sibling::node()) = '')">1.2em</xsl:when> -->
    <!-- 	<xsl:otherwise>0pt</xsl:otherwise> -->
    <!--   </xsl:choose> -->
    <!-- </xsl:attribute> -->
  </xsl:attribute-set>
  ...
  <xsl:param name="monospace.font.family"
    select="'Courier,monospace,ArialUnicode,Symbol,Times,LucidaSansUnicode,sans-serif,ZapfDingbats,CheltenhamCdITC,KozMinPro-Regular-Acro'"/>
  ...
  <xsl:attribute-set name="monospace.verbatim.properties"
    ...
    <xsl:attribute name="hyphenation-character">
      <xsl:choose>
        <xsl:when test="@role and string-length(@role) = 1">
          <xsl:value-of select="@role"/>
        </xsl:when>
        <xsl:otherwise>
          <!-- for JCBG-76 fop, alternate value for this when using fop -->
          <xsl:choose>
            <xsl:when test="$xep.extensions = 1">&#x21E6;</xsl:when>
            <xsl:otherwise>&#x00AB;</xsl:otherwise>
            <!--&#x00AB; works with fop but makes >>;  &#x21E6; works with XeP -->
            <!-- &#8617; -->
            <!-- &#x279D; -->
          </xsl:choose>
        </xsl:otherwise>
  ...
  <xsl:attribute-set name="orderedlist.label.properties"> </xsl:attribute-set>
  ...
  <xsl:param name="page.height.portrait">
    <xsl:choose>
      ...
      <xsl:when test="$paper.type = 'Motive7by9'">9in</xsl:when>
  ...
  <xsl:param name="page.margin.bottom" select="'.5in'"/>
  <xsl:param name="page.margin.inner">
    <xsl:choose>
      <xsl:when test="$double.sided != 0">1.375in</xsl:when>
      <xsl:otherwise>1in</xsl:otherwise>
    </xsl:choose>
  </xsl:param>
  <xsl:param name="page.margin.outer">
    <xsl:choose>
      <xsl:when test="$double.sided != 0">.875in</xsl:when>
      <xsl:otherwise>1in</xsl:otherwise>
    </xsl:choose>
  </xsl:param>
  <xsl:param name="page.margin.top" select="'.66in'"/>
  ...
  <xsl:param name="page.width.portrait">
    <xsl:choose>
      ...
      <xsl:when test="$paper.type = 'Motive7by9'">7in</xsl:when>
  ...
  <xsl:param name="paper.type">
    <xsl:choose>
      <xsl:when
        test="not(.//@lang) or .//@lang = '' or .//@lang[1] = 'en_US' or .//@lang[1] = 'en' or .//@lang[1] = 'xx_YY'"
        >USletter</xsl:when>
  ...
  <!-- changed below from 1 to 0 for JCBG-385 -->
  ...
  <xsl:attribute-set name="revhistory.table.properties"> </xsl:attribute-set>
  <xsl:attribute-set name="revhistory.table.cell.properties"> </xsl:attribute-set>
  <xsl:attribute-set name="revhistory.title.properties"> </xsl:attribute-set>
...
-->
  ...
  <!-- testing changes for JCBG-76, fop, was <xsl:param name="title.font.family" select="'CheltenhamCdITC,sans-serif,ZapfDingbats,KozMinPro-Regular-Acro'"/> -->
  <xsl:param name="title.font.family"
    select="'Trebuchet MS, Trebuchet, sans-serif,ZapfDingbats,KozMinPro-Regular-Acro'"/>
  ...
  <xsl:param name="toc.indent.width" select="9"/>
  <xsl:attribute-set name="toc.line.properties">
    <xsl:attribute name="text-align-last">justify</xsl:attribute>
    <xsl:attribute name="text-align">start</xsl:attribute>
    <xsl:attribute name="margin-left">.45in</xsl:attribute>
    <xsl:attribute name="font-size">12pt</xsl:attribute>

...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <!-- ********************************************************************
     $Id: titlepage.xsl,v 1.14 2009/08/26 15:10:52 dcramer Exp $
     ********************************************************************
  ...
  <xsl:template match="abstract/title" mode="titlepage.mode"> </xsl:template>
  ...
  <xsl:template match="biblioid" mode="titlepage.mode">
    <!-- only output on title page for some brandings (JCBG-133)- -->
    <xsl:if
      test="$branding = 'alcatel2' or $branding = 'motive2' ">
      <!-- or substring($branding,1,5) = 'nokia' --> 
      <span>
        <xsl:apply-templates select="." mode="class.attribute"/>
        <xsl:apply-templates mode="titlepage.mode"/>
        <br/>
  ...
  <xsl:template match="itermset" mode="titlepage.mode"> </xsl:template>
  ...
  <xsl:template match="keywordset" mode="titlepage.mode"> </xsl:template>

  <!-- for JCBG-689, changing this from matching on legalnotice[1] to legalnotice[last()] to this template so that it only triggers on the last legalnotice element in the document -->

  <xsl:template match="legalnotice[last()]" mode="titlepage.mode">
  ...
  <!-- define parameters that get passed in-->
  <xsl:param name="branding">motive</xsl:param>
  <xsl:param name="format"/>
  <xsl:param name="security"/>
  <!-- added for jcbg-133-->


  <!-- this template handles the content in MIDDLE of the title page -->
  <xsl:template name="biblioidAndRsa">
    <xsl:message terminate="no"> I'm using <xsl:value-of select="$branding"/> branding. </xsl:message>
    <!-- output a highlighted msg re: branding used, but only if security=reviewer (JCBG-133)-->
    <xsl:choose>
      <xsl:when test="$security='reviewer'">
        <p style="background-color:yellow;">Document generated with <xsl:value-of select="$branding"
          /> branding. </p>
      </xsl:when>
    </xsl:choose>
    <xsl:choose>
      <xsl:when test="$branding = 'motive'">

        <table cellpadding="5" cellspacing="0" width="100%" border="0">
          <tr>
            <td>
              <!-- not calling the title page template for productname, because I changed that to skip output of pubdate for some brandings-->
              <xsl:value-of select="../pubdate"/>
              <br/>
              <xsl:apply-templates select="../productname" mode="book.titlepage.recto.mode"/>

              <xsl:apply-templates select="../releaseinfo" mode="book.titlepage.recto.mode"/>
              <br/>
              <!-- not calling the title page template for biblioid, because I changed that to skip output of for some brandings-->
              <xsl:apply-templates select="../biblioid" mode="motive-bookinfo"/>
            ...
            <td style="text-align: right">
              <a href="http://www.rsasecurity.com" target="_blank">
                <xsl:choose>
                  <xsl:when test="$format = 'html'">RSA Secured</xsl:when>
                  <xsl:otherwise>
                    <img style="border-width: 0in" alt="RSA Secured logo"
                      src="./images/_common/RSA_Secured_logo.gif"/>
                  </xsl:otherwise>
                </xsl:choose>
              </a>
            </td> --> 
      ...
      <xsl:when test="$branding = 'alcatel'">
        <p>
          <xsl:apply-templates select="../productnumber" mode="alcatel-bookinfo"/>
          <xsl:apply-templates select="../productname" mode="alcatel-bookinfo"/>
          <xsl:apply-templates select="../releaseinfo" mode="alcatel-bookinfo"/>
          <br/>
          <xsl:apply-templates select="//book/title|//book/bookinfo/title" mode="alcatel-bookinfo"/>
          <xsl:apply-templates select="../edition" mode="alcatel-bookinfo"/>
          <xsl:apply-templates select="../pubdate" mode="alcatel-bookinfo"/>
          <xsl:apply-templates select="../invpartnumber" mode="alcatel-bookinfo"/>
        </p>
      </xsl:when>
      <xsl:otherwise>
        <!-- This orig listed the brandings where it applied; I added alcatel2 for JCBG-133. We're actually NOT going to output the biblioid here, because we already do 
          it earlier; it's just a useful spot to output RSA logo.
          
          So, this should trigger for all BUT alcatel, motive brandings: ie motive2, alcatel2, nokia1, and all future, but if need it NOT to, add a when above. 
          
          
           12/3/15 changing this to an otherwise so that ALL brandings other than those caught earlier get this bit, in an attempt to future-proof this.
        -->
        
        <!--  Hiding RSA b/c we no longer want to display it; here, this means this whole xsl:otherwise does nada.
        <table cellpadding="5" cellspacing="0" width="100%" border="0">
          <tr>
            <td>&#160; </td>
            <td style="text-align: right">
  ...
  <xsl:template match="biblioid" mode="motive-bookinfo">
    <!-- added this for JCBG-133, so that we could have a template that always outputs bibilioid -->
    <span>
      <xsl:apply-templates select="." mode="class.attribute"/>
      <xsl:apply-templates mode="titlepage.mode"/>
      <br/>
  ...
  <xsl:template match="*" mode="alcatel-bookinfo">
    <xsl:value-of select="."/>
    <br/>
  </xsl:template>
  ...
  <xsl:template match="modespec" mode="titlepage.mode"> </xsl:template>
  ...
  <xsl:template match="productname[parent::bookinfo]" mode="titlepage.mode">
    <span class="bookinfo-productname">
      <xsl:apply-templates mode="titlepage.mode"/>
      <br/>
    </span>
  </xsl:template>
  ...
  <xsl:template match="pubdate" mode="titlepage.mode">
    <!-- only output the pubdate on title page for some brandings (JCBG-133)- -->
  ...
  <xsl:template match="subjectset" mode="titlepage.mode"> </xsl:template>
  ...
  <xsl:template match="title" mode="titlepage.mode">
    ...
    <!-- new branding stuff for jcbg-133 
  
  Header info for the title page...
  -->

    <xsl:choose>
      <xsl:when test="$branding = 'motive' or $branding = 'alcatel'">
      ...
      <xsl:when test="$branding = 'alcatel2' or $branding = 'motive2'">
        <!-- Alcatel cover image and cover data inserted here, before the title
        
        Changes in this area to fix JCBG-777
        -->
        ...
        <xsl:choose>
          <xsl:when test="local-name(..) ='book' or local-name(..) ='bookinfo'">
            ...
            <img style="border-width: 0in;" alt="Big picture of a modern city"
              src="./images/_common/cityimage.png"/>

            <h1>
              <xsl:apply-templates select="." mode="class.attribute"/> ALCATEL-LUCENT <xsl:value-of
                select="//productnumber[1]"/></h1>
            ...
            <h2>
              ...
              <xsl:call-template name="string.upper">
                <xsl:with-param name="string">
                  <xsl:value-of select="//productname[1]"/> | <xsl:value-of
                    select="//releaseinfo[1]"/>
                </xsl:with-param>
              </xsl:call-template>
            ...
            <!-- and now here comes the title, reduced to h3... -->
            <h3>
              <xsl:apply-templates select="." mode="class.attribute"/>
              <a id="{$id}"/>
              <xsl:choose>
          ...
          <xsl:otherwise>
            ...
            </h1><xsl:comment>THIS H1 comes from titlepage.xsl, the otherwise case re: alc2/motive2</xsl:comment>
      ...
      <xsl:otherwise>
        <h1>xxxGlobal otherwise for titles @#@ used by all new brandings NEED TO CREATE OVERRIDE IN $branding.xsl</h1>
      </xsl:otherwise> 
    </xsl:choose>
  </xsl:template>

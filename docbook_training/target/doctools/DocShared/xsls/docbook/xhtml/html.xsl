...
<!-- ********************************************************************
     $Id: html.xsl,v 1.6 2010/01/25 17:41:53 dcramer Exp $
     ********************************************************************
...
<xsl:template name="href.target.with.base.dir">
  ...
  <!--  The dingbat and dingbat.characters templates provided below: commented these out 3/23/16 
    to fix JCBG-1839, Fix bad characters for trademark/(R)symbols in monolithic html outputs
    -Aaron DaMommio
-->    
...
<xsl:template name="dingbat.characters">
  ...
  <xsl:param name="dingbat">bullet</xsl:param>
  <xsl:choose>
    <xsl:when test="$dingbat='bullet'"><saxon:entity-ref name="#8226"/></xsl:when>
    <xsl:when test="$dingbat='copyright'"><saxon:entity-ref name="#169"/></xsl:when>
    <xsl:when test="$dingbat='trademark'"><saxon:entity-ref name="#8482"/></xsl:when>
    <xsl:when test="$dingbat='trade'"><saxon:entity-ref name="#8482"/></xsl:when>
    <xsl:when test="$dingbat='registered'"><saxon:entity-ref name="#174"/></xsl:when>
    <xsl:when test="$dingbat='service'">(SM)</xsl:when>
    <xsl:when test="$dingbat='nbsp'"><saxon:entity-ref name="#160"/></xsl:when>
    <xsl:when test="$dingbat='ldquo'"><saxon:entity-ref name="#8220"/></xsl:when>
    <xsl:when test="$dingbat='rdquo'"><saxon:entity-ref name="#8221"/></xsl:when>
    <xsl:when test="$dingbat='lsquo'"><saxon:entity-ref name="#8216"/></xsl:when>
    <xsl:when test="$dingbat='rsquo'"><saxon:entity-ref name="#8217"/></xsl:when>
    <xsl:when test="$dingbat='em-dash'"><saxon:entity-ref name="#8212"/></xsl:when>
    <xsl:when test="$dingbat='mdash'"><saxon:entity-ref name="#8212"/></xsl:when>
    <xsl:when test="$dingbat='en-dash'"><saxon:entity-ref name="#8211"/></xsl:when>
    <xsl:when test="$dingbat='ndash'"><saxon:entity-ref name="#8211"/></xsl:when>
    <xsl:otherwise>
      <saxon:entity-ref name="#8226"/>
    </xsl:otherwise>
  </xsl:choose>
</xsl:template> -->
...
<xsl:template name="id.warning">
  ...
  <xsl:param name="propagate.role.to.style"/>
...
<xsl:template match="*" mode="class.attribute">
  ...
  <xsl:attribute name="class">
	  ...
	  <xsl:if test="@role and not(@role = '') and ($propagate.role.to.style = '1' or $propagate.role.to.style = 'true')">

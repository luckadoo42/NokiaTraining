...
<xsl:template name="select.target.database">
  ...
  <xsl:variable name="target.database.filename">
    <xsl:choose>
      <xsl:when test="$xml.base != '' and
        ...
        <xsl:value-of select="translate($target.database.document,'\','/')"/>
      </xsl:when>
      <xsl:otherwise>
        <xsl:value-of select="translate($target.database.document,'\','/')"/>
      </xsl:otherwise>
  ...
  <xsl:if test="$olink.debug != 0">   
    <!-- display the derived filename -->
    <xsl:message><xsl:text>
    </xsl:text>
      target.database.document=<xsl:value-of select="$target.database.document"/>
    </xsl:message>
  </xsl:if>
  ...
  <xsl:if test="$olink.debug != 0">   
    ...
    <xsl:message><xsl:text>
      ...
      target.database.filename=<xsl:value-of select="$target.database.filename"/>
    </xsl:message>
  </xsl:if>
  
  <xsl:variable name="sanitized.target.database.filename" select="translate($target.database.filename,'\','/')"/>  <!-- added translate to make all slashes into / re: JCBG-76 fop -->
 
  <xsl:variable name="target.database" 
      select="document($sanitized.target.database.filename,/)"/> 

  <xsl:if test="$olink.debug != 0">   
    <!-- display the derived filename -->
    <xsl:message><xsl:text>
    </xsl:text>
      sanitized.target.database.filename=<xsl:value-of select="$sanitized.target.database.filename"/>
    </xsl:message>
  </xsl:if>
...
<!-- DWC: Adding this param to control hyperlinking of olinks -->
<xsl:param name="olink.hyperlink.external.docs" select="0"/>
...
<xsl:template name="make.olink.href">
  ...
  <!-- DWC: By default, don't hyperlink olinks...does this do anything? See also chunk-common.xsl -->

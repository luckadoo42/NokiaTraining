...
<!-- ********************************************************************
     $Id: stripns.xsl,v 1.1 2009/10/07 18:06:27 dcramer Exp $
     ********************************************************************
...
<xsl:template match="/*" mode="stripNS">
  <xsl:choose>
    <xsl:when test="self::ng:* or self::db:* or self::d:*">
...
<xsl:template match="db:info|d:info" mode="stripNS">
  <xsl:variable name="info">
    <xsl:choose>
      ...
      <xsl:when test="parent::db:audioobject
		              ...
		              |parent::d:audioobject
                      |parent::d:imageobject
                      |parent::d:inlinemediaobject
                      |parent::d:mediaobject
                      |parent::d:mediaobjectco
                      |parent::d:textobject
                      |parent::d:videoobject">
  ...
  <xsl:if test="(not(../db:title) and not(../d:title) and not(../ng:title))
    ...
    <xsl:apply-templates select="db:title|ng:title|d:title" mode="stripNS"/>
  </xsl:if>

</xsl:template>

<xsl:template match="ng:link|db:link|d:link" mode="stripNS">
...
<xsl:template match="ng:tag|db:tag|d:tag" mode="stripNS">
  <sgmltag>
    <xsl:copy-of select="@*[not(name(.) = 'xml:id')
                         and not(name(.) = 'version')]"/>
    <xsl:apply-templates mode="stripNS"/>
...
<xsl:template match="ng:textdata|db:textdata|d:textdata
                     |ng:imagedata|db:imagedata|d:imagedata
                     |ng:videodata|db:videodata|d:videodata
                     |ng:audiodata|db:audiodata|d:audiodata" mode="stripNS">
  <xsl:element name="{local-name(.)}">
    <xsl:copy-of select="@*[not(name(.) = 'xml:id')
                            and not(name(.) = 'version')
